...
class TrapcamAnalyzer:
    ...
    def count_flies(self, image_stack, frame_to_analyze, minimum_contour_size=50, maximum_contour_size=200, threshold=25):
    ...
    def fit_data_to_gamma_gauss_bimodal(self, data, expected, ax_handle, plot_histogram):
        ...
        def bimodal_gamma_gauss(x, gam_shape, gam_loc, gam_scale, A1, mu2,sigma2,A2): #gam_shape --> kurtosis, gam_loc --> x position of L edge, gam_scale --> some measure of dispersion
            gam_instance = gamma(gam_shape, gam_loc, gam_scale)
            gam = A1*gam_instance.pdf(x)
            return gam + gauss(x, mu2,sigma2,A2)

        try:
            params,cov=curve_fit(bimodal_gamma_gauss,x,y,expected) # <---- NEED ERROR HANDLING HERE
        except RuntimeError:
        ...
        fit = bimodal_gamma_gauss(x,*params)
        dy = np.diff(fit,1)
        dx = np.diff(x,1)
        y_first = dy/dx
        x_first = 0.5*(x[:-1]+x[1:])
        ...
        if plot_histogram:
            plot(x,bimodal_gamma_gauss(x,*params),color='black',lw=2,label='bimodal fit')
    ...
    def fit_data_to_trimodal(self,data,expected,ax_handle,plot_histogram):
        ...
        try:
            params,cov=curve_fit(trimodal,x,y,expected) # <---- NEED ERROR HANDLING HERE
        except RuntimeError:
            print ('optimal parameters not found')
            return None
        ...
        for key in d:
            ...
            if analysis_params["step through frames"]:
                ...
                #### now plotting the graph of flies over time
                    ...
                    # expected=(15,5,100,20,5,20,32,5,20)
                    # proposed_contrast_metric = self.fit_data_to_trimodal(all_contrast_metrics, expected, ax_handle = ax2,plot_histogram = True)
...
###############################################################
            ...
            if analysis_params["save frames as video"]:
                ...
                sample_video_str = self.directory +'/trapcam_timelapse/'+key+'/'+key + '_analyzed_%d_min_post_release' %(min_post_r)+'.mp4'
